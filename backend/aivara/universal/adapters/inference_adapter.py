"""Inference Execution Integrity Evidence Adapter (Phase 12.2 / Phase 10 Upstream).

Normalizes evidence from Inference Integrity:
- 18-checkpoint comprehensive inference verification results
- Input-model-preprocessing cryptographic binding hashes
- Replay consistency (exact and tolerant float bounds)
- Monotonic sequence and hash-linked record chain seals
"""
...
class InferenceIntegrityEvidenceAdapter(BaseEvidenceAdapter):
    """Adapter for Phase 10 Inference Integrity evidence records."""
    ...
    def normalize(
        ...
        # 2. Extract identity
        ev_id = str(raw.get("evidence_id") or raw.get("id") or ctx.get("evidence_id") or f"ev_inf_{uuid.uuid4().hex[:12]}")
        ev_type = str(raw.get("evidence_type") or "inference_verification_proof")
        ...
        asset_type = str(raw.get("target_asset_type") or "inference")
        inf_id = raw.get("inference_id") or raw.get("record_id") or raw.get("target_asset_id") or raw.get("affected_asset_id") or ctx.get("inference_id") or "unknown_inference"
        asset_id = str(inf_id)

        # 5. Ancestry extraction
        anc_dict = raw.get("ancestry_keys") or {}
        model_fp = raw.get("model_fingerprint") or raw.get("model_id") or anc_dict.get("model_fingerprint") or ctx.get("model_id")
        sample_id = raw.get("sample_id") or raw.get("input_hash") or anc_dict.get("sample_id")
        
        ancestry = AncestryPath(
            model_fingerprint=str(model_fp) if model_fp else None,
            sample_id=str(sample_id) if sample_id else None,
            extra_keys={k: str(v) for k, v in anc_dict.items() if k not in ("model_fingerprint", "sample_id")},
        )
        anc_status = AncestryStatus.VERIFIED if not ancestry.is_empty() else AncestryStatus.UNVERIFIED

        # 6. Payloads & Hashes
        data_json = raw.get("data_json") or raw.get("verification_results") or {k: v for k, v in raw.items() if k not in ("id", "evidence_id", "project_id", "source_payload_hash", "binding_hash", "record_hash", "artifact_hash")}
        supplied_src_hash = raw.get("source_payload_hash") or raw.get("binding_hash") or raw.get("record_hash") or raw.get("artifact_hash")
