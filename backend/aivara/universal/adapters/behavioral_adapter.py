"""Behavioral Analysis Evidence Adapter (Phase 12.2 / Phase 8 Upstream).

Normalizes evidence from Behavioral Analysis:
- Output prediction stability metrics under input perturbations
- Model behavioral drift from certified baseline executions
- Adversarial sensitivity and decision boundary curvature metrics
"""
...
class BehavioralAnalysisEvidenceAdapter(BaseEvidenceAdapter):
    """Adapter for Phase 8 Behavioral Analysis evidence records."""
    ...
    def normalize(
        ...
        # 2. Extract identity
        ev_id = str(raw.get("evidence_id") or raw.get("id") or ctx.get("evidence_id") or f"ev_ba_{uuid.uuid4().hex[:12]}")
        ev_type = str(raw.get("evidence_type") or raw.get("metric_name") or "behavioral_stability_metric")
