"""Contributor Risk Evidence Adapter (Phase 12.2 / Phase 6 Upstream).

Normalizes evidence from Contributor Risk:
- Contributor empirical Bayes anomaly vectors
- Leave-One-Out (LOO) subgroup divergence metrics
- Group specialization vs label skew confounding indicators
- Contributor pseudonymized identities
"""
...
class ContributorRiskEvidenceAdapter(BaseEvidenceAdapter):
    """Adapter for Phase 6 Contributor Risk evidence records."""
    ...
    def normalize(
        ...
        # 2. Extract identity
        ev_id = str(raw.get("evidence_id") or raw.get("id") or ctx.get("evidence_id") or f"ev_cr_{uuid.uuid4().hex[:12]}")
        ev_type = str(raw.get("evidence_type") or raw.get("dimension_id") or "contributor_anomaly_profile")
        ...
        asset_type = str(raw.get("target_asset_type") or "contributor")
        contrib_id = raw.get("contributor_id") or raw.get("target_asset_id") or raw.get("affected_asset_id") or ctx.get("contributor_id") or "unknown_contributor"
